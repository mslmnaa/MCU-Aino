...
<div class="text-center mb-8 border-b pb-4">
    ...
    <div class="grid grid-cols-2 gap-8 mt-6 text-left max-w-md mx-auto">
        <div>
            <p class="text-sm text-neutral-600 mb-1">Nama:</p>
            <p class="font-semibold text-neutral-800">{{ $patient->name }}</p>
        </div>
        <div>
            <p class="text-sm text-neutral-600 mb-1">Jenis Kelamin:</p>
            <p class="font-semibold text-neutral-800">{{ $patient->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
        </div>
        <div>
            <p class="text-sm text-neutral-600 mb-1">Umur:</p>
            <p class="font-semibold text-neutral-800">{{ $patient->umur }} tahun</p>
        </div>
        <div>
            <p class="text-sm text-neutral-600 mb-1">Tanggal:</p>
            <p class="font-semibold text-neutral-800">{{ date('d F Y') }}</p>
        </div>
    </div>
</div>
@endif

<!-- Menggunakan container yang lebih kecil untuk layout user -->
<div class="max-w-6xl mx-auto">
    <div class="mb-6 no-pdf">
        <div class="flex items-center justify-between">
            <div class="text-sm text-neutral-500">
                Patient Information • {{ $patient->departemen }} • {{ $patient->jabatan }}
            </div>
            <div class="flex space-x-3">
                <button onclick="testExport()" class="bg-red-500 hover:bg-red-600 text-neutral-50 px-6 py-2 rounded-lg font-medium transition-colors duration-200 flex items-center">
    ...
    <div class="flex items-center space-x-8">
        <div class="flex-shrink-0 relative">
            ...
            <button onclick="togglePhotoModal()"
                    class="no-pdf absolute -bottom-2 -right-2 w-8 h-8 bg-white rounded-full border-2 border-primary-300 flex items-center justify-center hover:bg-primary-50 transition-colors shadow-md">
...
<div id="photoModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold text-neutral-800">Change Profile Photo</h3>
                    <button onclick="togglePhotoModal()" class="text-neutral-400 hover:text-neutral-600">
                ...
                <div class="text-center mb-6">
                    @if($patient->profile_photo || $patient->user?->profile_photo)
                        <img src="{{ asset('storage/' . ($patient->profile_photo ?? $patient->user->profile_photo)) }}"
                             alt="{{ $patient->name }}"
                             class="w-24 h-24 rounded-full object-cover mx-auto border-4 border-neutral-200">
                    @else
                        <div class="w-24 h-24 rounded-full bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center mx-auto border-4 border-neutral-200">
                ...
                <!-- Upload Form -->
                <form action="{{ route('profile.upload-photo') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
                        ...
                        <input type="file" name="profile_photo" accept="image/*"
                               ...
                               required>
                        <p class="text-xs text-neutral-500 mt-1">PNG, JPG, GIF up to 2MB</p>
                    </div>

                    <div class="flex space-x-3">
                        <button type="submit" class="flex-1 bg-primary-600 hover:bg-primary-700 text-white py-2 px-4 rounded-lg font-medium transition-colors">
                            Upload Photo
                        </button>
                        @if($patient->profile_photo || $patient->user?->profile_photo)
                        <button type="button" onclick="deletePhoto()" class="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-lg font-medium transition-colors">
                            Delete
                        </button>
                        @endif
                    </div>
...
<script>
let activeYears = [{{ $availableYears->count() > 0 ? $availableYears->max() : 'null' }}].filter(year => year !== null);
const maxYears = 5;
...
function updateUnifiedSection() {
    ...
    // Generate header WITHOUT edit button (user view)
...
function exportSelectedYears() {
    // Check if there are active years
    if (activeYears.length === 0) {
        showNotification('No years selected for export', 'warning');
        return;
    }

    // Get currently active years
    const yearsParam = activeYears.join(',');
    const exportUrl = `{{ route('my-health') }}?export=1&years=${yearsParam}`;
    ...
    // Use window.open for PDF download (better for PDF files)
